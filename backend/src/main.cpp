...
int main() {
    ...
    std::cout << "[config] Listen:    " << config.host << ":" << config.port << std::endl;
    std::cout << "[config] Cache:     " << config.cache_size << " entries" << std::endl;
    ...
    try {
        GraphLoader loader(
            std::make_unique<CSVStationParser>(),
            std::make_unique<CSVConnectionParser>());

        static Graph graph = loader.load(config.stations_path(),
                                         config.connections_path());
        g_graph = &graph;

        std::cout << "[init] Graph loaded: "
                  << graph.station_count() << " stations, "
                  << graph.connection_count() << " connections" << std::endl;
    ...
    std::cout << "\n[server] Starting City-Mapper API on "
    ...
    std::cout << "  POST /api/route             - Compute shortest route" << std::endl;
    std::cout << std::endl;
